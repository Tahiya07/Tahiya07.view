  ...
  return (
    <>
      ...
      <div className="hidden sm:flex fixed top-5 left-0 right-0 justify-center z-50 px-4">
        <div className="relative flex gap-2 px-3 py-2 rounded-full bg-white/5 border border-white/10 backdrop-blur-2xl shadow-lg">

          {/* glow base */}
          <div className="absolute inset-0 rounded-full bg-white/5 blur-xl opacity-40" />
      ...
      <div className="sm:hidden fixed bottom-5 left-0 right-0 flex justify-center z-50 px-4">
        <div className="flex gap-4 px-5 py-3 rounded-full bg-black/40 border border-white/10 backdrop-blur-2xl">

          {sections.map((sec) => (
            <a
              key={sec}
              href={`#${sec}`}
              className={`text-[11px] capitalize transition ${
                active === sec ? "text-white" : "text-white/40"
              }`}
            >
              {sec}
